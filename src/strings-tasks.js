...
function countVowels(str) {
  return str.split('').reduce((res, char) => {
    ...
    return (res += /[aeiouyAEIOUY]/.test(char) ? 1 : 0);
  }, 0);
}
...
function isPalindrome(str) {
  const onlyWordCharactersStr = str.replaceAll(/[^A-Za-z]/g, '').toLowerCase();
  const reversedStr = onlyWordCharactersStr.split('').reverse().join('');
  return onlyWordCharactersStr === reversedStr;
}
